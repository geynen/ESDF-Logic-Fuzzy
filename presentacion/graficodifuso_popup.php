...
$membresia=utf8_encode($dato->descripcion);
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
				chart = new Highcharts.Chart({
					...
					title: {
						text: '<?php echo ('Función de Membresia: '.$membresia);?>'
					},
					subtitle: {
					...
					tooltip: {
						formatter: function() {
							if(this.series.name=='CENTROIDE'){
								return '<b>'+ this.series.name +'</b><br><?php echo utf8_encode('Grado de decisi&oacute;n:');?> ' + this.x + '<br><?php echo $pertenencia;?>';
							}else{
					...
					foreach($variablesgraf as $indice => $v){?>
						{
						name: '<?php echo utf8_encode($v['nombre']);?>',
