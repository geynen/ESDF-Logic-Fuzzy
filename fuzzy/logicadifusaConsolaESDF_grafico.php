...
//class FuzzySEPER{
	...
	echo "ESDF Logic Fuzzy: Modo Consola";
		...
		while ($datorpta=$rstrpta->fetchObject()){
			...
			$dataGrafico.= "g.addNode('".$datorpta->membresia."',{ fill : '#56f'});";
		}
			...
			echo $v.'<br>';
				$rstregla=$objRegla->consultarMembresiasHija(str_replace('_',' ',$membresia));
				  while($dato=$rstregla->fetchObject()){
					  $dataGrafico.= "g.addEdge('".str_replace('_',' ',$membresia)."','".$dato->membresia_input1."',{ fill : '#56f' });";
					  $dataGrafico.= "g.addEdge('".str_replace('_',' ',$membresia)."','".$dato->membresia_input2."');";					  				  }
			}
		}
			...
			foreach($fuzzy_arr as $membresia => $v){
				if($v!=0){
					...
					if($membresia==$membresiafinal) $valormembresiafinal=$v;
						$rstregla=$objRegla->consultarMembresiasHija(str_replace('_',' ',$membresia));
					  while($dato=$rstregla->fetchObject()){
						  $dataGrafico.= "g.addEdge('".str_replace('_',' ',$membresia)."','".$dato->membresia_input1."');";
						  $dataGrafico.= "g.addEdge('".str_replace('_',' ',$membresia)."','".$dato->membresia_input2."');";					  				 	  }
				}
			}
		...
		$_SESSION['DataGrafico']=$dataGrafico;
