...
require("cls_usuario.php");
	...
	try{
		...
		if($_POST['txtUsuario']!='' and $_POST['txtClave']!=''){
			$ObjUsuario->actualizarUsuario($_POST['txtIdPersona'],$_POST['txtUsuario'], md5($_POST['txtClave']));
		}
		
		$cnx->commit(); 
		if($_SESSION['IdTipo']==1) header ("location: ../presentacion/main.php");
		if($_SESSION['IdTipo']==2) header ("location: ../presentacion/main_usuario.php");
		return 1;
	} catch (Exception $e) { 
		$cnx->rollBack(); 
		echo "Error de Proceso en Lotes: " . $e->getMessage();
...
if($accion=='LOGEO') {
	...
	$sql="SELECT idusuario,U.idpersona,login,clave,U.idtipousuario, TU.descripcion as tipousuario,P.foto FROM usuario U inner join 
	persona P on P.idpersona=U.idpersona inner join tipo_usuario TU on TU.idtipousuario=U.idtipousuario
	...
	if($cant==1){
		...
		$_SESSION['IdTipo']=$registro->idtipousuario;
